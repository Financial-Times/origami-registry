...
function Header(rootEl) {
	...
	const hierarchicalNavEls = [
			rootEl.querySelector('.o-registry-header__nav--primary-theme')
		].filter(function(el) {
			/**
	...
	function init() {
		...
		rootEl.setAttribute('data-o-registry-header--js', '');
	...
	function destroy() {
		...
		rootEl.removeAttribute('data-o-registry-header--js');
	}
...
Header.init = function(el) {
	...
	const headerEls = el.querySelectorAll('[data-o-component="o-registry-header"]');
	const headers = [];
	for (let c = 0, l = headerEls.length; c < l; c++) {
		if (!headerEls[c].hasAttribute('data-o-registry-header--js')) {
